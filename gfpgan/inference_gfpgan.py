...
def main():
    ...
    parser.add_argument(
        "--upscale", type=int, default=2, help="The final upsampling scale of the image"
    )
    parser.add_argument(
        "--arch",
        type=str,
        default="clean",
        help="The GFPGAN architecture. Option: clean | original",
    )
    parser.add_argument(
        "--channel",
        type=int,
        default=2,
        help="Channel multiplier for large networks of StyleGAN2",
    )
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    parser.add_argument(
        "--paste_back",
        action="store_false",
        help="Paste the restored faces back to images",
    )
    parser.add_argument(
        "--save_root", type=str, default="results", help="Path to save root"
    )
    parser.add_argument(
        "--ext",
        type=str,
        ...
        try:
            ...
            for idx, (cropped_face, restored_face) in enumerate(
                ...
                save_crop_path = os.path.join(
                    args.save_root, "cropped_faces", f"{basename}_{idx:02d}.png"
                )
                ...
                save_restore_path = os.path.join(
                    args.save_root, "restored_faces", save_face_name
                )
                ...
                imwrite(
                    cmp_img,
                    os.path.join(args.save_root, "cmp", f"{basename}_{idx:02d}.png"),
                )
            ...
            if restored_img is not None:
                ...
                if args.suffix is not None:
                    ...
                    save_restore_path = os.path.join(
                        args.save_root, "restored_imgs", f"{basename}.{extension}"
                    )
                imwrite(restored_img, save_restore_path)
        
        except:
            print(f"Unable to process {img_name}!!!")
